...
export const POST = adminRoute('partnership_manager', async ({ req, ctx, admin, params }) => {
  ...
  const p = { p_id: id, p_manager: (typeof body.managerId==='string'?body.managerId:null) };
  const data = await callAdminRpc(admin, 'assign_introduction_manager', Object.assign({ p_request_id: ctx.requestId }, p));
