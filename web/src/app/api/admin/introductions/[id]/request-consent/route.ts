...
export const POST = adminRoute('partnership_manager', async ({ req, ctx, admin, params }) => {
  ...
  const p = { p_id: id, p_target_user: (typeof body.targetUserId==='string'?body.targetUserId:null) };
  const data = await callAdminRpc(admin, 'request_target_consent', Object.assign({ p_request_id: ctx.requestId }, p));
