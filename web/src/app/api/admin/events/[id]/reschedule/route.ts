...
export const POST = adminRoute('event_manager', async ({ req, ctx, admin, params }) => {
  ...
  void body;
  const data = await callAdminRpc(admin, 'reschedule_event', Object.assign({ p_request_id: ctx.requestId }, { p_id: params.id, p_starts: body.starts, p_ends: body.ends ?? null, p_reason: body.reason ?? null }));
