...
export const POST = adminRoute('event_manager', async ({ req, ctx, admin, params }) => {
  ...
  void body;
  const data = await callAdminRpc(admin, 'cancel_event', Object.assign({ p_request_id: ctx.requestId }, { p_id: params.id, p_public_reason: String(body.publicReason ?? ''), p_internal_note: body.internalNote ?? null }));
