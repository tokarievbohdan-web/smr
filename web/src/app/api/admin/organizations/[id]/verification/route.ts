...
const ALLOWED = new Set(['verified', 'rejected', 'pending', 'unverified']);
...
export const POST = adminRoute('moderator', async ({ req, ctx, admin, params }) => {
  ...
  const data = await callAdminRpc(admin, 'admin_set_org_verification', { p_id: id, p_status: status, p_note: note ?? null, p_request_id: ctx.requestId });
