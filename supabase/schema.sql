-- ============================================================
-- ⚠️ DEPRECATED — НЕ застосовувати як production-механізм.
-- Джерело правди схеми з Milestone 1 — послідовні міграції у
--   supabase/migrations/2026072812000X_*.sql
-- Застосування: `supabase db push` (див. supabase/README.md → runbook).
-- Цей файл лишено як історичний reference одного-файлового знімка;
-- він МІСТИТЬ ВІДОМІ ВАДИ, виправлені в міграціях:
--   · is_admin() без SECURITY DEFINER → рекурсія RLS на admin_users;
--   · profiles select віддавав email усім (витік);
--   · власник міг самоверифікуватися (verified) і публікувати в обхід модерації;
--   · немає soft delete (deleted_at) і типізованих дат.
-- Не редагуйте цей файл для змін БД — додавайте нову міграцію.
-- ============================================================
-- Sport Market Review — схема бази даних (Supabase / Postgres) — reference.
-- ============================================================
